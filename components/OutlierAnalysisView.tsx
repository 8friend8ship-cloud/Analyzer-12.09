...
import { addToCollection, createOutlierCollectionItem } from '../services/collectionService';
...
const OutlierVideoRow: React.FC<{ video: VideoData; averageViews: number; onShowVideoDetail: (id: string) => void; onShowChannelDetail: (id: string) => void }> = ({ video, averageViews, onShowVideoDetail, onShowChannelDetail }) => {
    ...
    return (
        <div className="grid grid-cols-12 items-center gap-4 px-4 py-3 hover:bg-gray-700/40">
            ...
            <div className="col-span-3 md:col-span-2 text-center">
                 <p className="font-bold text-lg text-blue-400">{multiplier.toFixed(1)}배</p>
                <p className="text-xs text-gray-400">평균 대비</p>
            </div>
            <div className="col-span-3 md:col-span-2 text-center">
            ...
            <div className="col-span-3 md:col-span-2 text-center">
                <p className="font-bold text-lg text-green-400">${formatNumber(video.estimatedRevenue)}</p>
                <p className="text-xs text-gray-400">추정 수익</p>
            </div>
        </div>
    );
};
...
const OutlierAnalysisView: React.FC<OutlierAnalysisViewProps> = ({ user, appSettings, onBack, onShowVideoDetail, onShowChannelDetail, onUpgradeRequired, onUpdateUser, savedState, onSaveState }) => {
    ...
    const handleExcludeCategoryChange = useCallback((categoryId: string, checked: boolean) => {
        setExcludedCategories(prev => {
            ...
            if (checked) newSet.add(categoryId);
        ...
        try {
            const apiKey = user.isAdmin ? appSettings.apiKeys.youtube : (user.apiKeyYoutube || appSettings.apiKeys.youtube);
             ...
             const trendingVideos = await fetchRankingData('videos', {
                 limit: 50, country: trendingCountry, category: 'all', metric: 'mostPopular', excludedCategories: Array.from(excludedCategories)
             }, apiKey);
             
             if (trendingVideos.length === 0) throw new Error("트렌드 데이터를 가져올 수 없습니다.");
             ...
             const calculatedTopChannels = Array.from(channelStats.values())
                .sort((a, b) => b.count !== a.count ? b.count - a.count : b.totalViews - a.totalViews)
                .slice(0, 10).map(stat => stat.name);

             const videoTitles = trendingVideos.map(v => ({ title: v.name, channelTitle: (v as any).channelName || '' }));
             const excludedLabels = EXCLUDABLE_CATEGORIES.filter(cat => excludedCategories.has(cat.id)).map(cat => cat.label);
             const insight = await getAITrendingInsight(trendingCountry, videoTitles, excludedLabels, calculatedTopChannels);
             
             const finalTrendingData = { ...insight, topChannels: calculatedTopChannels };
             setTrendingData(finalTrendingData);
             
             // [BIZ] Auto-save to Vault
             if (user.plan === 'Biz' || user.isAdmin) {
                 addToCollection({
                     id: `trend_${trendingCountry}_${Date.now()}`,
                     type: 'trend',
                     title: `${countryOptions.find(c => c.value === trendingCountry)?.label} 실시간 트렌드 리포트`,
                     thumbnailUrl: '',
                     metric1: `키워드 ${insight.topKeywords.length}개`,
                     metric2: `요약 생성됨`,
                     date: new Date().toISOString(),
                     url: '',
                     raw: finalTrendingData
                 });
             }
    ...
    }, [trendingCountry, user, appSettings, excludedCategories]);

    const handleAnalysis = useCallback(async (e?: React.FormEvent) => {
        if (e) e.preventDefault();
        const planLimits = { Free: appSettings.freePlanLimit, Pro: appSettings.plans.pro.analyses, Biz: appSettings.plans.biz.analyses };
        ...
        try {
            ...
            if (mode === 'keyword' && trendingCountry !== 'KR' && trendingCountry !== 'WW' && /[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/.test(query)) {
                try { searchQuery = await translateKeyword(query, trendingCountry); } catch (err) {}
            }

            const filters: FilterState = {
                resultsLimit: 100, sortBy: 'viewCount', period: '90', country: trendingCountry, minViews: 1000, videoLength: 'any', videoFormat: 'any', category: 'all',
            };
            ...
            const res = { videos: videoData, avgViews: averageViews };
            setAnalysisResult(res);

            // [BIZ] Auto-save to Vault
            if (user.plan === 'Biz' || user.isAdmin) {
                addToCollection(createOutlierCollectionItem({
                    query, mode, analysisResult: res, trendingCountry, trendingData, excludedCategories: Array.from(excludedCategories), multiplier
                }));
            }
    ...
    }, [user, appSettings, query, mode, onUpgradeRequired, onUpdateUser, trendingCountry, trendingData, excludedCategories, multiplier]);

    const { outlierVideos, displayStats } = useMemo(() => {
        if (!analysisResult) return { outlierVideos: [], displayStats: null };
        const { videos, avgViews } = analysisResult;
        if (videos.length === 0) return { outlierVideos: [], displayStats: { totalVideos: 0, averageViews: 0, outlierThreshold: 0, outlierCount: 0 } };
        ...
        return { outlierVideos: filtered, displayStats: { totalVideos: videos.length, averageViews: avgViews, outlierThreshold, outlierCount: filtered.length } };
    }, [analysisResult, multiplier]);

    return (
        <div className="p-4 md:p-6 lg:p-8">
            <button onClick={onBack} className="mb-4 px-4 py-2 text-sm font-semibold rounded-md bg-gray-600 hover:bg-gray-500">← 워크플로우로 돌아가기</button>
            <header className="text-center mb-8">
                <h1 className="text-3xl font-bold text-white">아웃라이어 & 트렌드 분석</h1>
                <p className="text-gray-400 mt-2 max-w-2xl mx-auto">평균을 뛰어넘는 '아웃라이어' 영상의 성공 비결을 분석하세요.</p>
            </header>

            <div className="mb-10 bg-gray-800/40 border border-gray-700 rounded-xl p-6">
                 <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
                    <div className="flex items-center gap-2"><span className="text-xl">🌍</span><h2 className="text-xl font-bold text-white">오늘의 국가별 트렌드 디스커버리</h2></div>
                        ...
                        <div className="flex items-center gap-3 bg-gray-700/50 px-3 py-1.5 rounded-md border border-gray-600/50">
                            ...
                            {EXCLUDABLE_CATEGORIES.map(cat => (
                                <label key={cat.id} className="flex items-center gap-1.5 cursor-pointer text-xs text-gray-300 hover:text-white select-none">
                                    <input type="checkbox" checked={excludedCategories.has(cat.id)} onChange={(e) => handleExcludeCategoryChange(cat.id, e.target.checked)} className="form-checkbox h-3.5 w-3.5 bg-gray-700 border-gray-600 rounded text-blue-600" />
                                    {cat.label}
                                </label>
                            ))}
                        </div>
                        <div className="flex items-center gap-2">
                            <select value={trendingCountry} onChange={(e) => setTrendingCountry(e.target.value)} className="bg-gray-700 border-gray-600 rounded-md p-2 text-sm text-white">
                                {countryOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.flag} {opt.label}</option>)}
                            </select>
                            <Button onClick={handleFetchTrending} disabled={isTrendingLoading} className="text-sm py-2">
                 ...
                 {trendingData && (
                     <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
                         <div className="lg:col-span-3 bg-blue-900/20 border border-blue-500/30 p-4 rounded-lg">
                             <h3 className="font-bold text-blue-400 mb-2 flex items-center gap-2"><span className="text-xl">🧠</span> AI 트렌드 인사이트</h3>
                             <p className="text-gray-200 text-sm leading-relaxed mb-3">{trendingData.summary}</p>
                             <div className="flex flex-wrap gap-2">{trendingData.viralFactors.map((factor, i) => <span key={i} className="px-2 py-1 bg-blue-800/50 text-blue-200 text-xs rounded-full border border-blue-600/50">#{factor}</span>)}</div>
                         </div>
                         <div className="bg-gray-800/80 p-4 rounded-lg">
                             <h3 className="font-bold text-yellow-400 mb-3 flex items-center gap-2">🔥 급상승 키워드 Top 10</h3>
                             <div className="flex flex-wrap gap-2">{trendingData.topKeywords.map((kw, i) => <button key={i} onClick={() => {setQuery(kw); setMode('keyword'); }} className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-white text-sm rounded-md transition-colors"><span className="text-yellow-500 font-bold mr-2">{i+1}.</span> {kw}</button>)}</div>
                         </div>
                         <div className="bg-gray-800/80 p-4 rounded-lg lg:col-span-2">
                             <h3 className="font-bold text-green-400 mb-3 flex items-center gap-2">📺 트렌드 주도 채널 Top 10</h3>
                             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{trendingData.topChannels.map((ch, i) => <div key={i} className="flex items-center justify-between bg-gray-700/50 p-2 rounded-md"><span className="font-medium text-gray-200 truncate"><span className="text-green-500 font-bold mr-2">{i+1}.</span> {ch}</span><button onClick={() => { setQuery(ch); setMode('channel'); handleAnalysis(); }} className="text-xs text-blue-400 hover:underline">분석</button></div>)}</div>
                         </div>
                     </div>
                 )}
            </div>

            <form onSubmit={handleAnalysis} className="max-w-3xl mx-auto mb-8 flex flex-col sm:flex-row gap-2 bg-gray-900 p-4 rounded-lg border border-gray-700 shadow-lg">
                ...
                <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={mode === 'keyword' ? "키워드 (예: 'AI 그림')" : "채널 URL 또는 ID"} className="flex-grow bg-gray-700 border-gray-600 rounded-md p-3 text-white focus:ring-blue-500" />
                <Button type="submit" disabled={isLoading} className="px-6 py-3">{isLoading ? "분석 중..." : "심층 분석"}</Button>
            </form>

            {isLoading && <div className="flex justify-center py-20"><Spinner /></div>}
            {!isLoading && !error && analysisResult && (
                <div className="space-y-6">
                    <div className="bg-gray-800/60 p-4 rounded-lg border border-gray-700/50 text-center">
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-700/50 flex items-center justify-center gap-4">
                            <label className="text-sm text-gray-300">배수: <span className="text-white font-bold">{multiplier}배</span></label>
                            <input type="range" min="2" max="20" step="1" value={multiplier} onChange={e => setMultiplier(parseInt(e.target.value, 10))} className="w-full max-w-xs h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer" />
                        </div>
                    </div>
                    {outlierVideos.length > 0 && (
                        <div className="bg-gray-800/60 rounded-lg border border-gray-700/50 overflow-hidden">
                            <div className="divide-y divide-gray-700/50">{outlierVideos.map(video => <OutlierVideoRow key={video.id} video={video} averageViews={displayStats!.averageViews} onShowVideoDetail={onShowVideoDetail} onShowChannelDetail={onShowChannelDetail} />)}</div>
                        </div>
                    )}
                </div>
            )}
