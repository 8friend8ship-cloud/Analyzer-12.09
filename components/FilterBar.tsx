...
const FilterBar: React.FC<FilterBarProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // But if the country is KR, we should NOT translate even if the keyword is in English (user might want English results in Korea)
    // Actually, the issue reported is "Korean search mixed with foreign videos".
    // This usually happens if relevanceLanguage is not set correctly or if translation happens when it shouldn't.
  ...
  return (
    <div className="bg-gray-900/80 backdrop-blur-sm p-3 border-b border-gray-700/50 flex-shrink-0">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center gap-3 mb-3">
        {/* Search Tabs */}
        <div className="flex-shrink-0 flex items-center gap-2 w-full md:w-auto overflow-x-auto no-scrollbar">
            <button onClick={() => onSearchTabChange('video')} className={`flex-1 md:flex-none px-4 py-2 text-sm font-semibold rounded-md whitespace-nowrap ${searchTab === 'video' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
                영상 탐색 (Video Search)
            </button>
            <button onClick={() => onSearchTabChange('channel')} className={`flex-1 md:flex-none px-4 py-2 text-sm font-semibold rounded-md whitespace-nowrap ${searchTab === 'channel' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
                채널 탐색 (Channel Search)
            </button>
        </div>
        {/* Search Form */}
        <form onSubmit={handleSubmit} className="w-full md:flex-grow flex items-center gap-2">
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm">
          <div className="flex items-center gap-1.5">
              <span className="text-sm font-semibold text-gray-400 mr-1">필터:</span>
              <label htmlFor="country" className="text-gray-300 hidden sm:inline">국가:</label>
              <select id="country" value={filters.country} onChange={e => handleFilterChange('country', e.target.value)} className="bg-gray-700 border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-xs p-1.5 max-w-[120px]">
                  {countryOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
              </select>
          </div>
          <div className="flex items-center gap-1.5">
              <label htmlFor="category" className="text-gray-300 hidden sm:inline">카테고리:</label>
              <select 
                  id="category" 
                  value={filters.category} 
                  onChange={e => handleFilterChange('category', e.target.value)} 
                  className="bg-gray-700 border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-xs p-1.5 max-w-[120px]"
              >
                  {YOUTUBE_CATEGORY_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
              </select>
          </div>
          <div className="flex items-center gap-1.5">
              <label htmlFor="sortBy" className="text-gray-300 hidden sm:inline">정렬:</label>
           ...
           <div className="flex items-center gap-1.5">
              <label htmlFor="resultsLimit" className="text-gray-300 hidden sm:inline">결과:</label>
          ...
          <div className={`flex items-center gap-1.5 ${searchTab === 'video' ? 'flex' : 'hidden'}`}>
              <label htmlFor="minViews" className="text-gray-300 hidden sm:inline">최소 조회수:</label>
          ...
          <div className={`flex items-center gap-1.5 ${searchTab === 'video' ? 'flex' : 'hidden'}`}>
              <label htmlFor="videoLength" className="text-gray-300 hidden sm:inline">길이:</label>
          ...
          <div className={`flex items-center gap-1.5 ${searchTab === 'video' ? 'flex' : 'hidden'}`}>
              <label htmlFor="period" className="text-gray-300 hidden sm:inline">기간:</label>
          ...
          <div className={`flex items-center gap-1.5 ${searchTab === 'video' ? 'flex' : 'hidden'}`}>
              <label className="text-gray-300 hidden sm:inline">포맷:</label>
          ...
          <div className="flex-grow flex justify-end gap-2 w-full sm:w-auto mt-2 sm:mt-0">
              <Button 
                  type="button" 
                  onClick={onCompare} 
