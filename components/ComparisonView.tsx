...
const ResultsTable: React.FC<ResultsTableProps> = ({ videos, onShowChannelDetail, onOpenCommentModal, selectedChannels, onChannelSelect }) => {
                                ...
                                <tr key={video.id} className="hover:bg-gray-800/50">
                                    <td className="px-4 py-3 align-top">
                                        <div className="flex items-start space-x-3">
                                            ...
                                            <img className="w-32 h-[72px] rounded-md object-cover flex-shrink-0" src={video.thumbnailUrl} alt={video.title} />
                                            <div className="flex-grow">
                                                <p className="font-semibold text-white line-clamp-2 leading-snug mb-1">{video.title}</p>
                ...
                {videos.map((video, index) => (
                    <div key={video.id} className="bg-gray-800/80 rounded-lg p-3 border border-gray-700/50">
                        <div className="flex items-start space-x-3 mb-3">
                            ...
                            <img className="w-28 h-[63px] rounded-md object-cover flex-shrink-0" src={video.thumbnailUrl} alt={video.title} />
                            <div className="flex-grow">
                                <p className="font-semibold text-white text-sm line-clamp-2 leading-snug mb-1">{video.title}</p>
                                <div className="flex items-center gap-1.5">
                                    <p className="text-xs text-gray-400 truncate">{video.channelTitle}</p>
                                    <button onClick={() => handleCopy(video.channelTitle, video.id)} className="text-gray-500 hover:text-white transition-colors duration-150 flex-shrink-0" title="채널명 복사">
                        ...
                        <div className="flex justify-between items-center mt-3 pt-3 border-t border-gray-700/70">
                            ...
                            <div className="flex items-center space-x-1.5 w-full max-w-[240px]">
                               <button className="px-2 py-1.5 text-xs font-semibold rounded bg-purple-600 hover:bg-purple-700 text-white flex-1 text-center" onClick={() => window.open(`https://www.youtube.com/watch?v=${video.id}`, '_blank')}>정보</button>
                               <button className="px-2 py-1.5 text-xs font-semibold rounded bg-gray-600 hover:bg-gray-500 text-white flex-1 text-center" onClick={() => onOpenCommentModal({id: video.id, title: video.title})}>댓글 분석</button>
