              ...
              <div className="space-y-8">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <StatCard 
                        ...
                        subValue={data.subscriberCount < 1000 ? "수익 창출 미달" : "최근 트래픽 기준"}
                        highlight={true}
                    />
                    <StatCard 
                        title="총 누적 수익 (추산)" 
                        value={`$${formatNumber(data.estimatedTotalRevenue, true)}`}
                        subValue={data.subscriberCount < 1000 ? "수익 창출 미달" : "전체 영상 누적"}
                    />
                     <StatCard 
                        title="최근 30일 업로드" 
                ...
                <section>
                    <h2 className="text-2xl font-bold mb-4">AI 채널 건강도 진단</h2>
                ...
                <section>
                 ...
                 <section>
                     <h2 className="text-2xl font-bold mb-4">채널 주요 토픽 분석 (최근 10개 영상 기준)</h2>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-gray-800 p-4 rounded-lg">
                            <h3 className="font-semibold mb-3">인기 키워드 (빈도 분석)</h3>
                             <ul className="space-y-2">
                                {data.overview.popularKeywords.map(({ keyword, score }) => (
                                  <li key={keyword} className="flex items-center justify-between text-sm">
                                    ...
                                    <div className="w-24 h-2.5 bg-gray-700 rounded-full"><div className="h-2.5 bg-blue-500 rounded-full" style={{width: `${score}%`}}></div></div>
                                  </li>
                                ))}
                             </ul>
                        </div>
                         <div className="bg-gray-800 p-4 rounded-lg">
                            <h3 className="font-semibold mb-3">주요 태그/토픽</h3>
                             <div className="flex flex-wrap gap-2">
                                {data.overview.competitiveness.tags.slice(0, 15).map(tag => <span key={tag} className="px-2.5 py-1 text-xs bg-gray-700 text-gray-200 rounded-full">{tag}</span>)}
                             </div>
                        </div>
                     </div>
                </section>
