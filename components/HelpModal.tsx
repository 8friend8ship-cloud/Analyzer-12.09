      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <HelpSection title="API 키 설정" icon={<KeyIcon />}>
            <p>콘텐츠 OS의 모든 기능은 관리자가 설정한 시스템 API 키를 통해 작동합니다. 헤더의 'API Status' 아이콘으로 현재 상태를 확인할 수 있습니다.</p>
            <ul className="list-disc list-outside pl-5 space-y-1">
                <li><b>필수 API 키:</b>
                    <ul className="list-decimal list-outside pl-5 mt-1 space-y-1">
                        <li><b>YouTube Data API:</b> 영상 및 채널의 기본 데이터를 수집합니다.</li>
                        <li><b>Gemini API:</b> AI 기능(연관 키워드, 심층 분석 등)에 사용됩니다.</li>
                    </ul>
                </li>
                <li><b>설정 방법 (관리자 전용):</b>
                    <ul className="list-decimal list-outside pl-5 mt-1 space-y-1">
                        <li><b>관리자</b>는 '관리자' 메뉴에서 시스템 전체에 적용될 공용 API 키를 설정할 수 있습니다.</li>
                        <li><b>일반 사용자</b>는 별도의 키 설정이 필요 없으며, 문제가 발생할 경우 관리자에게 문의해야 합니다.</li>
                    </ul>
                </li>
                <li><b>상태 확인:</b> 헤더의 'API Status' 아이콘이 <span className="text-green-400 font-semibold">녹색</span>이면 모든 키가 정상 설정된 상태이며, <span className="text-red-400 font-semibold">빨간색</span>이면 키가 누락된 상태입니다. 문제 발생 시 관리자(8friend8ship@hanmail.net)에게 문의해주세요.</li>
            </ul>
          </HelpSection>
        </div>
      </div>
