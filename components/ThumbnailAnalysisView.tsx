...
const ThumbnailAnalysisView: React.FC<ThumbnailAnalysisViewProps> = ({ user, appSettings, onBack, savedState, onSaveState }) => {
  ...
  const handleAnalysis = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      // FIX: The 'scoredThumbnails' property does not exist on AIThumbnailInsights type,
      // and the associated feature is disabled for compliance. Removing scoring logic.
      ...
      {!isLoading && !error && (
        isInitial ? (
          ...
          <div className="space-y-8">
            ...
            <div>
              <h2 className="text-xl font-bold mb-4">참고 영상</h2>
                ...
                {thumbnails.map(video => (
                  <a key={video.id} href={`https://www.youtube.com/watch?v=${video.id}`} target="_blank" rel="noopener noreferrer" className="group block relative">
                    ...
                    <div className="aspect-video bg-gray-800 rounded-lg overflow-hidden transition-transform group-hover:scale-105">
                      <img src={(video as any).thumbnailUrl} alt={(video as any).title} className="w-full h-full object-cover" />
                    </div>
                    <p className="text-xs text-gray-400 mt-2 line-clamp-2 group-hover:text-white">{(video as any).title}</p>
                  </a>
                ))}
              </div>
            </div>
