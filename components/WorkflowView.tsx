...
const TrendIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" /></svg>;
...
const workflowSteps = [
    ...
    },
    {
        ...
        features: [
            ...
            { id: 'trend_ranking', title: "트렌드 랭킹 (Google & YouTube)", description: "Google 검색 트렌드와 YouTube 급상승 순위를 통합하여, 지금 뜨는 소재를 실시간으로 발굴", icon: <TrendIcon /> }
        ]
    },
    {
        step: 4,
    ...
    },
    {
        step: 5,
        features: [
            { id: 'algorithm_finder', title: "알고리즘 주제 찾기 (Algorithm Finder)", description: "질문 선택만으로 나만의 카테고리, 연령, 성향, 키워드 패턴을 자동 추출하여 방향성 제시", icon: <AlgorithmIcon />, isBizOnly: true },
            { id: 'collections', title: "컬렉션 (Collections)", description: "여러 도구에서 찾은 아이디어·데이터·결과물을 한 곳에 모아 체계적으로 정리", icon: <CollectionsIcon />, isBizOnly: true }
        ]
    }
];

const FeatureCard: React.FC<{ feature: typeof workflowSteps[0]['features'][0]; onNavigate: (featureId: string) => void; }> = ({ feature, onNavigate }) => (
    <button onClick={() => onNavigate(feature.id)} className="bg-gray-800/60 rounded-lg p-4 flex items-start gap-4 border border-gray-700/50 w-full text-left hover:bg-gray-700/50 transition-colors group relative overflow-hidden">
        ...
        <div>
            <div className="flex items-center gap-2 mb-1">
                ...
                {(feature as any).isBizOnly && (
                    <span className="flex items-center gap-1 bg-purple-900/50 border border-purple-500/30 text-purple-300 text-[10px] px-1.5 py-0.5 rounded font-semibold uppercase">
                        <LockIcon /> Biz
                    </span>
                )}
            </div>
        ...
        <div className="p-4 md:p-6 lg:p-8">
            <div className="max-w-4xl mx-auto">
                <header className="text-center mb-10">
                    <h1 className="text-3xl font-bold text-white">Johnson 워크플로우</h1>
                        ...
                        {workflowSteps.map((step, index) => (
                            <div key={step.step} className="flex flex-col md:flex-row items-center justify-center relative">
                                <div className="md:w-1/2 flex md:justify-end md:pr-12 mb-4 md:mb-0 w-full">
                                    {step.features[0] && <FeatureCard feature={step.features[0]} onNavigate={onNavigate} />}
                                </div>
                                ...
                                <div className="md:w-1/2 flex md:justify-start md:pl-12 w-full">
                                     {step.features[1] && <FeatureCard feature={step.features[1]} onNavigate={onNavigate} />}
                                </div>
