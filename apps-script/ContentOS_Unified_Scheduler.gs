...
function contentOsUnifiedSchedulerTick() {
  const out = {ok:true,version:CONTENTOS_UNIFIED_SCHEDULER_VERSION,startedAt:new Date().toISOString(),stages:{}};
...
function runOptionalContentOsStage_(handlerName) {
  ...
  } catch (err) { return {ok:false,handler:handlerName,error:String(err&&err.message||err)}; }
}

function runContentOsScheduledStagesFromFactory(){return contentOsUnifiedSchedulerTick();}

function auditContentOsTriggerContract(){
  const triggers=ScriptApp.getProjectTriggers();
  const rows=triggers.map(function(t){return{handler:t.getHandlerFunction(),source:String(t.getTriggerSource()),eventType:String(t.getEventType())};});
  ...
  const duplicateImageSupply=rows.filter(function(r){return r.handler==='runImageSupplyGovernor10mV1_'||r.handler==='runImageSupplyPreLearningV2_'||r.handler==='runImageSupplyPostLearningV2_'||r.handler==='runImageSupplyPriorityGovernorV1_'||r.handler==='runGeneratedImageReingestV1_'||r.handler==='runImageDailyTargetAutoScaleV1_';}).length;
  ...
  return {ok:duplicateOwn<=1&&duplicateAllApp===0&&duplicateImage===0&&duplicateImageSupply===0&&duplicateApiAudit===0&&centralSheetAudit<=1&&workflowBridgeCrosscheck===0&&openAi5Physical===0,physicalTriggerPolicy:'REUSE_EXISTING_FACTORY_PROCESS_TASK_QUEUE;OPENAI5_LOGICAL_ONLY;NO_OPENAI5_PHYSICAL_TRIGGER',unifiedTriggerCount:duplicateOwn,allAppPhysicalTriggerCount:duplicateAllApp,imageLearningPhysicalTriggerCount:duplicateImage,imageSupplyPhysicalTriggerCount:duplicateImageSupply,apiCredentialAuditPhysicalTriggerCount:duplicateApiAudit,centralSheetAuditTriggerCount:centralSheetAudit,workflowBridgeCrosscheckPhysicalTriggerCount:workflowBridgeCrosscheck,openAi5PhysicalTriggerCount:openAi5Physical,openAi5LogicalMinutes:5,triggers:rows,version:CONTENTOS_UNIFIED_SCHEDULER_VERSION};
}

function testContentOsApiFreeVirtualFrontX2(){
  if(typeof enqueueContentOsQuery!=='function'||typeof contentOsPipelineTick!=='function')return{ok:false,reason:'PIPELINE_FUNCTIONS_NOT_SYNCED',version:CONTENTOS_UNIFIED_SCHEDULER_VERSION};
  const ss=SpreadsheetApp.openById(CONTENTOS_PIPELINE_SHEET_ID),log=ss.getSheetByName('Pipeline_Log');if(!log)return{ok:false,reason:'Pipeline_Log missing'};
  const before=log.getLastRow(),suffix=Utilities.formatDate(new Date(),'Asia/Seoul','yyyyMMddHHmmss'),f1=enqueueContentOsQuery('APP_CONTENT_OS','두바이 쫀득 쿠키 '+suffix,20),r1=contentOsPipelineTick(),middle=log.getLastRow();Utilities.sleep(50);const f2=enqueueContentOsQuery('APP_CONTENT_OS','신라면 먹방 '+suffix,20),r2=contentOsPipelineTick(),after=log.getLastRow(),run1Added=middle>before,run2Added=after>middle,pipelinePass=run1Added&&run2Added&&r1&&r1.ok!==false&&r2&&r2.ok!==false,readiness=typeof testContentOsVirtualFrontReadinessX2==='function'?testContentOsVirtualFrontReadinessX2():{ok:false,reason:'VIRTUAL_FRONT_QA_NOT_SYNCED'},pass=pipelinePass&&readiness&&readiness.ok===true;return{ok:pass,API_FREE_FINAL_PASS:pass,pipelinePass:pipelinePass,readinessPass:readiness&&readiness.ok===true,run1Added:run1Added,run2Added:run2Added,logDelta:after-before,fixtures:[f1,f2],runs:[r1,r2],readiness:readiness,version:CONTENTOS_UNIFIED_SCHEDULER_VERSION};
}
