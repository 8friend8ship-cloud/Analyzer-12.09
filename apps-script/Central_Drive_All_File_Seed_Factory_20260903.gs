...
/**
 * Logical-only all-Drive ingestion lane.
...
function runCentralDriveAllFileSeedFactoryFromFactory() {
  ...
  if (!lock.tryLock(1000)) return {ok:true, skipped:true, reason:'LOCKED', version:CENTRAL_DRIVE_ALL_FILE_SEED_VERSION};
  const started = Date.now();
  try {
    const props = PropertiesService.getScriptProperties();
    const tokenKey = 'CENTRAL_DRIVE_ALL_FILE_ITERATOR_TOKEN';
    const sweepKey = 'CENTRAL_DRIVE_ALL_FILE_SWEEP_ID';
    let sweepId = props.getProperty(sweepKey) || ('SWEEP_' + Utilities.formatDate(new Date(),'Asia/Seoul','yyyyMMdd_HHmmss'));
    let it;
    const token = props.getProperty(tokenKey);
    try { it = token ? DriveApp.continueFileIterator(token) : DriveApp.getFiles(); }
    catch (e) { it = DriveApp.getFiles(); props.deleteProperty(tokenKey); sweepId = 'SWEEP_' + Utilities.formatDate(new Date(),'Asia/Seoul','yyyyMMdd_HHmmss'); }
    ...
    const seedSheet = ss.getSheetByName(CENTRAL_DRIVE_ALL_FILE_SEED_REGISTRY);
    const queensSheet = ss.getSheetByName(CENTRAL_DRIVE_ALL_FILE_QUEENS);
    let processed = 0, unchanged = 0, failed = 0;
    const runId = 'DRIVESEED_' + Utilities.formatDate(new Date(),'Asia/Seoul','yyyyMMdd_HHmmss');

    while (it.hasNext() && processed + unchanged + failed < CENTRAL_DRIVE_ALL_FILE_BATCH && Date.now() - started < CENTRAL_DRIVE_ALL_FILE_BUDGET_MS) {
      const f = it.next();
      try {
        if (f.isTrashed()) { unchanged++; continue; }
        ...
        if (existing && String(existing.dedupKey) === dedupKey && String(existing.result) === 'SEED_REGISTERED') { unchanged++; continue; }
    ...
    if (it.hasNext()) props.setProperty(tokenKey, it.getContinuationToken());
    else { props.deleteProperty(tokenKey); props.setProperty(sweepKey, 'SWEEP_' + Utilities.formatDate(new Date(),'Asia/Seoul','yyyyMMdd_HHmmss')); }

    let downstream = {ok:true, skipped:true, reason:'NO_ROUTER'};
    if (typeof centralProcessNewSeeds === 'function') {
      try { downstream = centralProcessNewSeeds(); } catch (e) { downstream = {ok:false,error:String(e && e.message || e)}; }
    }
    return {ok:failed===0, runId:runId, sweepId:sweepId, processed:processed, unchanged:unchanged, failed:failed, more:it.hasNext(), downstream:downstream, version:CENTRAL_DRIVE_ALL_FILE_SEED_VERSION};
...
function ensureDriveAllFileRuntimeSheet_(ss) {
  ...
  const headers = ['RUN_ID','CHECKED_AT','FILE_ID','FILE_NAME','MIME_TYPE','LAST_UPDATED','SIZE_BYTES','DATA_CLASS','PROJECT_ID','QUEENS_STATUS','SEED_STATUS','SEED_ID','TEMPLATE_ROUTE','BRIDGE_ROUTE','DEDUP_KEY','RESULT','ACK','DRIVE_READBACK','RETRY_STATE','ERROR','SOURCE_URL','WORKFLOW_ID','RUNTIME_STATE','NOTES'];
...
function findRuntimeByFileId_(sh, fileId) {
  ...
  for (let i=vals.length-1;i>=0;i--) if (String(vals[i][2]) === String(fileId)) return {dedupKey:vals[i][14],result:vals[i][15],row:start+i};
  return null;
}

function upsertRuntimeRow_(sh, x) {
  const row = [x.runId,x.checkedAt,x.fileId,x.fileName,x.mimeType,x.lastUpdated,x.sizeBytes,x.dataClass,x.projectId,x.queensStatus,x.seedStatus,x.seedId,x.templateRoute,x.bridgeRoute,x.dedupKey,x.result,x.ack,x.driveReadback,x.retryState,x.error,x.sourceUrl,x.workflowId,x.runtimeState,x.notes];
...
function upsertByHeaderBestEffort_(sh, keyHeader, keyValue, obj, successLabel) {
  ...
  const lastCol = sh.getLastColumn(); if (lastCol < 1) return 'TARGET_HEADER_MISSING';
  const headers = sh.getRange(1,1,1,lastCol).getValues()[0].map(String);
  const keyCol = headers.indexOf(keyHeader); if (keyCol < 0) return 'TARGET_KEY_HEADER_MISSING';
  let rowNo = 0; const lastRow = sh.getLastRow();
  if (lastRow >= 2) {
    const vals = sh.getRange(2,keyCol+1,lastRow-1,1).getValues();
    for (let i=0;i<vals.length;i++) if (String(vals[i][0]) === String(keyValue)) { rowNo=i+2; break; }
  }
  const row = new Array(lastCol).fill('');
  headers.forEach(function(h,i){ if (Object.prototype.hasOwnProperty.call(obj,h)) row[i]=obj[h]; });
  if (rowNo) sh.getRange(rowNo,1,1,lastCol).setValues([row]); else sh.appendRow(row);
  return successLabel;
}
...
function shortHashDriveSeed_(s) { const d=Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256,String(s)); return d.slice(0,10).map(b=>('0'+((b+256)%256).toString(16)).slice(-2)).join('').toUpperCase(); }
function safeDriveFileSize_(f) { try { return f.getSize(); } catch (_) { return ''; } }
function safeFn_(fn,fallback) { try { return fn(); } catch (_) { return fallback; } }
...
function testCentralDriveAllFileSeedFactoryForceX2() {
  ...
  return {ok:ok, first:a, second:b, version:CENTRAL_DRIVE_ALL_FILE_SEED_VERSION};
}

/** Deprecated: all-file Seed routing must reuse existing factory wake. */
function installCentralDriveAllFileSeedTrigger() {
  return {ok:false, blocked:true, reason:'NO_NEW_PHYSICAL_TRIGGER_REUSE_PROCESS_TASK_QUEUE', version:CENTRAL_DRIVE_ALL_FILE_SEED_VERSION};
}
