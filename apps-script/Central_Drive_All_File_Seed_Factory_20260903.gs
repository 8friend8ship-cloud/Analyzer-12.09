  ...
  try {
    ...
    while (
      ...
      try {
        ...
        const queensResultId = 'QRES_DRIVE_' + shortHashDriveSeed_(fileId + '|QUEENS');
        ...
        const queensStatus = upsertQueensCandidateBestEffort_(queensSheet, {
          ...
          APP_ID:projectId,
          RESEARCH_TYPE:'DRIVE_FILE_CONTENT_EXTRACTION_PENDING',
          ...
          MARKET_ID:'INTERNAL',
          LOCALE_ID:'und',
          EVIDENCE_STATUS:'METADATA_ONLY_CONTENT_QA_REQUIRED',
          SEED_STATUS:'QA_PENDING_CONTENT_EXTRACTION',
          SOURCE_HASH:shortHashDriveSeed_(dedupKey),
          NOTES:'FILE_ID=' + fileId + ';MIME=' + mime + ';DATA_CLASS=' + dataClass + ';TEMPLATE_ROUTE=' + route.template + ';BRIDGE_ROUTE=' + route.bridge + ';PUBLIC_YN=N;NO_METADATA_ONLY_SEED=TRUE'
        });

        const intakeOk = queensStatus === 'QUEENS_REGISTERED';
        upsertRuntimeRow_(runtime, {
          ...
          dedupKey:dedupKey,
          result:intakeOk ? 'QUEENS_REGISTERED_PENDING_SEED_QA' : 'QUEENS_REGISTRATION_FAILED',
          ack:intakeOk ? 'ACK' : 'NO_ACK',
          driveReadback:intakeOk ? 'PASS' : 'FAIL',
          retryState:intakeOk ? 'WAIT_CONTENT_EVIDENCE' : 'RETRY_NEXT_WAKE',
          error:intakeOk ? '' : ('QUEENS=' + queensStatus),
          sourceUrl:sourceUrl,
        ...
        if (intakeOk && typeof centralPublishDataEvent === 'function') {
          try {
            centralPublishDataEvent({
              ...
              entity_id:queensResultId,
              summary:name,
              source_url:sourceUrl,
              lineage_ids:[fileId,queensResultId],
...
function upsertQueensCandidateBestEffort_(sh, obj) {
  return upsertByHeaderBestEffort_(sh,'RESULT_ID',obj.RESULT_ID,obj,'QUEENS_REGISTERED');
}
